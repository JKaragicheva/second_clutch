...
# model
for (row in 1:nrow) {
log(lamda[row])<- Intercept+InterceptFirstClutchDouble*FirstClutchDoubleBreeders[row]+ # Intercept fixed
...
plot(jitter(Res$Clutch)~jitter(Res$DayFirst), pch=".", cex=2,
      xlab="Time (days)", ylab="Clutch size", las=1, type="n")
...
# model
for (row in 1:nrow) {
log(lamda[row])<- Intercept+w_int_double*InterceptFirstClutchDouble*FirstClutchDoubleBreeders[row]+
...
Data <- list("nrow"=nrow(Res), "nyears"=nyears, "year"=Res$Year,
             "DayFirst"=Res$DayFirst, "DayDiff"=Res$DayDiff, "Clutch"=Res$Clutch,
             "FirstClutchDoubleBreeders"=Res$FirstClutchDoubleBreeders)
