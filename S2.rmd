...
## Main model w/o model selection
### Bayesian model
```{r}
...
# model
for (row in 1:nrow) {
log(lamda[row])<- Intercept+Intercept_first_clutch_double*FirstClutchDoubleBreeders[row]+InterceptYear[year[row]]+ #intercept
...
Data <- list("nrow"=nrow(Res), "nyears"=nyears, "year"=Res$Year, "DayFirst"=Res$DayFirst, "DayDiff"=Res$DayDiff, "Clutch"=Res$Clutch, "FirstClutchDoubleBreeders"=Res$FirstClutchDoubleBreeders)

inits <- function() list("Intercept" = runif(1,-1,3),  "Intercept_first_clutch_double"= runif(1,-1,3), "InterceptYear" = runif(nyears,-1,1), "Slope" =-runif(1,-0.5, 0.5),"SlopeYear" =runif(nyears,-1,1), "SlopeSecondClutch" =runif(1,-1,1), "sigma_intercept"=runif(1, 0,0.5), "sigma_slope"=runif(1,0,1))

# Parameters monitored
params <- c("Intercept", "Intercept_first_clutch_double", "InterceptYear", "Slope", "SlopeYear", "SlopeSecondClutch", "sigma_intercept", "sigma_slope")
```
...
out <- jags.parallel(Data, inits, params, "two_truncated_Poisson_model.jags", n.chains = 5, n.thin = 500, n.iter = 250000, n.burnin = 50000)
...
print(out_ms)
#plot original data
plot(jitter(log(Res$Clutch))~jitter(Res$DayFirst), pch=".", cex=2)

# ok, now overall slope and intercept
Intercept_pred<-quantile(out_ms$BUGSoutput$sims.list$Intercept, c(0.025, 0.5, 0.075)) 
Slope_pred<-quantile(out_ms$BUGSoutput$sims.list$Slope, c(0.025, 0.5, 0.075)) 

XX<-seq(min(Res$DayFirst), max(Res$DayFirst))
Y_main<-Intercept_pred[2]+Slope_pred[2]*XX
lines(Y_main~XX, lwd=2)
# now CI

Y_lower<-apply(out_ms$BUGSoutput$sims.list$Slope %*% XX + out_ms$BUGSoutput$sims.list$Intercept[,1], 2, quantile, c(0.025))

Y_upper<-apply(out_ms$BUGSoutput$sims.list$Slope %*% XX + out_ms$BUGSoutput$sims.list$Intercept[,1], 2, quantile, c(0.975))

Y_second_clutch<-apply(out_ms$BUGSoutput$sims.list$Slope %*% XX + out_ms$BUGSoutput$sims.list$Intercept[,1]+out_ms$BUGSoutput$sims.list$Intercept_first_clutch_double[,1]+out_ms$BUGSoutput$sims.list$SlopeSecondClutch %*% XX, 2, quantile, c(0.5))

Y_second_clutch_upper<-apply(out_ms$BUGSoutput$sims.list$Slope %*% XX + out_ms$BUGSoutput$sims.list$Intercept[,1]+out_ms$BUGSoutput$sims.list$Intercept_first_clutch_double[,1]+out_ms$BUGSoutput$sims.list$SlopeSecondClutch %*% XX, 2, quantile, c(0.975))
Y_second_clutch_lower<-apply(out_ms$BUGSoutput$sims.list$Slope %*% XX + out_ms$BUGSoutput$sims.list$Intercept[,1]+out_ms$BUGSoutput$sims.list$Intercept_first_clutch_double[,1]+out_ms$BUGSoutput$sims.list$SlopeSecondClutch %*% XX, 2, quantile, c(0.025))
lines(Y_lower~XX)
lines(Y_upper~XX)
lines(Y_second_clutch~XX, col="red", lwd=2)
lines(Y_second_clutch_lower~XX, col="red")
lines(Y_second_clutch_upper~XX, col="red")

# and now prediction for the second clutches..
# this will be second clutch intercept
plot(jitter(Res$Clutch)~jitter(Res$DayFirst), pch=".", cex=2)
lines(exp(Y_main)~XX, lwd=2)
lines(exp(Y_upper)~XX)
lines(exp(Y_second_clutch)~XX, col="red", lwd=2)
lines(exp(Y_second_clutch_lower)~XX, col="red")
lines(exp(Y_second_clutch_upper)~XX, col="red")

hist(out_ms$BUGSoutput$sims.list$Intercept_first_clutch_double)
hist(out_2_trunc$BUGSoutput$sims.list$sigma2)
```
...
# model
for (row in 1:nrow) {
log(lamda[row])<- Intercept+w_int_double*Intercept_first_clutch_double*FirstClutchDoubleBreeders[row]+w_int*InterceptYear[year[row]]+ #intercept
...
library(R2jags)
# Call JAGS from R (few hours)
out_ms <- jags.parallel(Data, inits, params, "two_truncated_Poisson_model_with_MS.jags", n.chains = 5, n.thin = 500, n.iter = 250000, n.burnin = 50000)
